...
**PuMarket** 是一个基于 Spring Boot 的企业级拼团电商平台，采用领域驱动设计（DDD）架构思想，实现了完整的拼团交易链路。系统支持多种优惠策略（打折、直减、满减等），提供完善的营销管理后台，并通过 Kafka 消息队列处理异步事件，具备良好的可扩展性与高可用性。
